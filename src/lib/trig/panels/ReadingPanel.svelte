<script lang="ts">
	...
	// Some text to read. `onContinue` acknowledges it and moves the flow forward.
	let { panel, onContinue }: { panel: Panel; onContinue: () => void } = $props();
</script>
...
<div class="scroll-chill h-full overflow-y-auto px-8 py-12">
	<div class="mx-auto flex min-h-full max-w-lg flex-col [justify-content:safe_center]">
		...
		<h1 class="mb-5 text-3xl font-bold text-brand-charcoal">{panel.title}</h1>
